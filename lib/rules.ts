...
export function analyzeTaskEvidence(status: string, evidence: GithubEvidence[]): TaskAnalysis {
  if (!evidence.length) return { code:"insufficient_evidence", severity:"info", summary:"沒有 Issue 或 PR 證據，無法判斷開發階段。" };
  ...
  if (status === "已完成" && (blocked || (active && !merged) || (issueOpen && !merged))) {
    return { code:"status_conflict", severity:"conflict", summary:"Notion 已完成，但 GitHub 仍有未完成或阻塞證據。" };
  }
  if ((status === "未開始" || status === "執行中") && merged) {
    return { code:"possibly_complete", severity:"warning", summary:"PR 已合併；可能完成，但仍需確認部署、QA 或驗收。" };
  }
  if (status === "未開始" && active) {
    return { code:"likely_in_progress", severity:"warning", summary:"已有 PR 開發活動；Notion 可能應更新為執行中。" };
  }
  return { code:"consistent", severity:"none", summary:"目前沒有明確的狀態矛盾。" };
}
